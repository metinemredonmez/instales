...
| `daily_bars(market, symbols, start) -> list[Bar]` | `ingestion/prices.load_prices` → `market_prices` | `symbols` are **instrument symbols** (`ASELS`); the provider maps them to its convention and skips what it cannot map. `Bar.symbol` is the instrument symbol again. |
...
2. **REST history endpoint** — path and parameters for daily OHLCV; date range limits per call; whether adjusted and unadjusted series are both available; pagination.
