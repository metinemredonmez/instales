...
import { fmtCountdown, fmtOpensAt, fmtQuoteChange, fmtQuotePrice, marketStateDetail, marketStateLabel, minutesUntil, nextMarketState, quoteTone, trTimeSuffix } from "./quotes"
...
describe("market-status countdown", () => {
  ...
  it("knows which state follows when next_change_at passes", () => {
    expect(nextMarketState("TR", "open")).toBe("closed")
    expect(nextMarketState("TR", "closed")).toBe("open")
    expect(nextMarketState("US", "closed")).toBe("pre")
    expect(nextMarketState("US", "pre")).toBe("open")
    expect(nextMarketState("US", "open")).toBe("post")
    expect(nextMarketState("US", "post")).toBe("closed")
  })
  ...
  it("builds the pill text per state", () => {
    const open: MarketStatus = { state: "open", label_key: "market.open", next_change_at: "2026-09-16T15:00:00Z", tz: "Europe/Istanbul" }  // 18:00 Istanbul
    expect(marketStateLabel("TR", "open", tTr)).toBe("BIST açık")
    expect(marketStateDetail(open, now, tTr, "tr")).toBe("kapanışa 4s 12dk")

    const closed: MarketStatus = { state: "closed", label_key: "market.closed", next_change_at: "2026-09-17T07:00:00Z", tz: "Europe/Istanbul" }
    ...
    const closedUs: MarketStatus = { state: "closed", label_key: "market.closed", next_change_at: "2026-09-17T08:00:00Z", tz: "America/New_York" }  // pre-market 04:00 NY
    expect(marketStateDetail(closedUs, now, tTr, "tr")).toBe("Per 04:00'te açılır")

    const pre: MarketStatus = { state: "pre", label_key: "market.pre", next_change_at: "2026-09-16T13:30:00Z", tz: "America/New_York" }
    ...
    const post: MarketStatus = { state: "post", label_key: "market.post", next_change_at: "2026-09-17T00:00:00Z", tz: "America/New_York" }  // 20:00 NY
