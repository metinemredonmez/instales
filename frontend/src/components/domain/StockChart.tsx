...
export function StockChart({ symbol, market }: { symbol: string; market: Market }) {
  ...
  return (
    <Section title="Fiyat vs kurumsal pozisyon" hint="aynı zaman ekseni · ortak imleç">
      <div className="grid gap-0 px-2 py-3">
        <Panel title="Fiyat" change={priceChange} unit="₺">
          <ResponsiveContainer width="100%" height={150}>
            <LineChart data={prices} syncId={symbol} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
              ...
              <Tooltip content={<Tip label="Kapanış" fmt={(v) => `₺${v}`} />} cursor={{ stroke: "var(--muted-foreground)", strokeDasharray: "3 3" }} />
        ...
        <Panel title="Kurumsal toplam pozisyon (lot)" change={holdChange} unit="">
          <ResponsiveContainer width="100%" height={150}>
            <AreaChart data={holdings} syncId={symbol} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
              ...
              <Tooltip content={<Tip label="Lot" fmt={(v, p) => `${fmtLots(v).replace("+", "")} · ${p?.funds ?? "?"} fon`} />} cursor={{ stroke: "var(--muted-foreground)", strokeDasharray: "3 3" }} />
  ...
  return (
    <div>
      <div className="flex items-baseline gap-2 px-2 text-xs">
        ...
        <span className="text-muted-foreground">dönem başından</span>
      </div>
      {children}
    </div>
  )
