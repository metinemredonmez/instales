...
export function AiNoteCard({ market, symbol, title }: { market: Market; symbol?: string; title: string }) {
  ...
  const key = ["ai-note", market, symbol ?? "market"]
  const q = useQuery({ queryKey: key, queryFn: () => (symbol ? api.stockAi(market, symbol) : api.brief(market)), retry: false, staleTime: 5 * 60_000 })
  const refresh = async () => { setBusy(true); try { const n = symbol ? await api.stockAi(market, symbol, true) : await api.brief(market, true); qc.setQueryData(key, n) } finally { setBusy(false) } }
  const note = q.data as AiNote | null | undefined
  if (q.isLoading) return <div className="rounded-lg border border-border bg-card p-4 text-sm text-muted-foreground">AI notu hazırlanıyor…</div>
  if (!note) return null
  return (
    <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
      ...
      <div className="mt-2 text-[10px] uppercase tracking-wider text-muted-foreground">Betimleyici AI notu · yatırım tavsiyesi değildir · rakamlar InstiLens verisinden</div>
    </div>
  )
}
