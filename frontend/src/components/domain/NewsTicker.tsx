      ...
      {open && (
        ...
        <div className="relative my-2 rounded-lg border border-border bg-popover p-4 text-sm shadow-xl">
          <div className="absolute right-2 top-2 flex items-center gap-1 text-muted-foreground">
            {pinned ? <span className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider"><Pin className="size-3" /> sabit</span> : <span className="text-[10px] uppercase tracking-wider">tıkla → sabitle</span>}
            <button onClick={close} aria-label="Kapat" className="rounded p-1 hover:bg-accent hover:text-foreground"><X className="size-4" /></button>
          </div>
...
function TickerLink({ item, onEnter, onPin, children }: { item: Item; onEnter: () => void; onPin: () => void; children: React.ReactNode }) {
  ...
  if (item.kind === "news" && item.news) {
    ...
    return <a href={item.news.url} target="_blank" rel="noreferrer" onMouseEnter={onEnter} onFocus={onEnter} onContextMenu={pin} className={cls} title="Tıkla: kaynağa git · Sağ tık: kartı sabitle">{children}</a>
  }
  if (item.event) {
