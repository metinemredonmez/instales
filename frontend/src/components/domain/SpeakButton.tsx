...
export function SpeakButton({ noteId, text, lang }: { noteId: number; text: string; lang: "tr" | "en" }) {
  ...
  const play = async () => {
    ...
    if (status.data?.provider) {
      ...
      a.onerror = () => { setPlaying(false); setErr(t("tts.serverError", { p: status.data?.provider ?? "" })) }
      setPlaying(true)
      await a.play().catch(() => { setPlaying(false); setErr(t("tts.serverError", { p: status.data?.provider ?? "" })) })
    } else speak()
  }
  const speak = () => {
  ...
  return (
    <span className="inline-flex items-center gap-1">
      ...
      <button onClick={play} className="inline-flex items-center gap-1 rounded-md border border-border bg-card px-2 py-1 text-xs hover:bg-accent" title={provider ? `${t("tts.title")} (${provider})` : `${t("tts.title")} (${t("tts.browser")})`}>
        {playing ? <Pause className="size-3.5" /> : <Volume2 className="size-3.5" />} {playing ? t("tts.stop") : t("tts.listen")}
      </button>
