...
def get_stock_ai(symbol: str, market: str = MarketParam, refresh: bool = False, lang: str = LangParam, session: Session = Depends(get_session)):
    """AI note for the stock (flows × headlines). Cached per day and language; refresh=true regenerates."""
...
@router.get("/brief")
def get_brief(market: str = MarketParam, refresh: bool = False, lang: str = LangParam, session: Session = Depends(get_session)):
    """Morning brief for the market. Generated by the scheduler at 08:30 (TR + EN); refresh=true regenerates now."""
